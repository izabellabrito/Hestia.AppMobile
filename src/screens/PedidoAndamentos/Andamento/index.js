...
export default function PedidosAndamento({ navigation }) {
	...
	async function obtemPedidos() {
		...
		await api
			.get(`/pedido/obter-pedidos-em-andamento/${id}`, { headers: { Authorization: `Bearer ${jwt}` } })
			.then((response) => {
	...
	return (
		<Card>
			...
			<View style={{ alignItems: 'center' }}>
				<Botao onPress={() => navigation.navigate('Andamento', { id: item.id })}>
